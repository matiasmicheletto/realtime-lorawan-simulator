...
void Heu::ordenar(vector<vector<pii> >& eds,int c){
...
void Heu::cantGWTotal(int maxIter){
	int cantCanales = _inst-> cantCanales();
	int solTotal = 0;
	_sol = vector<int>(cantCanales);
	for(int c=0; c<cantCanales;c++){
		cantGW(maxIter,c);
		solTotal += _sol[c];
	}
	cout << "SolTotal: " << solTotal << endl;
}

void Heu::cantGW(int maxIter,int c){
	cout << "-----------------------------------------------------" << endl;
	cout << "Canal " << c+1 << ": " << endl;
	int n = _inst->n(c);
        int m = _inst->m(c);
	...
	int cont2 = _inst->cont2(c);
	int cont3 = _inst->cont3(c);
	int cont4 = _inst->cont4(c);
	...
	for(int iter=0; iter<maxIter;iter++){
		...
		int sol = asignar(c,eds[opcion],solGW,solk,solh,mapa,mapaED,gwabiertos);
		if(sol< _sol[c]){ //si encuentro un sol mejor
			...
			_solh = solh;
			_mapa = mapa;
			cout << "Sol: " << iter << " " << _sol[c] << "\t" << get_time(start) << endl;
		}
	}

	if(_sol[c]<=m) mostrarSol(c);
	/*if(_sol<=_m){
	...
	*/
	...
	if(_sol[c]<=m) cout << "Mejor Sol: " << _sol[c] << endl;
	else cout << "NO se encontro sol" << endl;
}


int Heu::asignar(int c, vector<pii>& eds, vector<vector<int> >& solGW, vector<vector<int> >& solk, vector<vector<int> >& solh,
	vector<vector<vector<int> > >& mapa, vector<vector<vector<bool> > >& mapaED, vector<bool>& gwabiertos){
	...
	gwabiertos = vector<bool>(m,false);
	//vector<bool> gwabiertos(m,false);
	int cantGW = 0;
	...
	for(int ip=0; ip<n; ip++){
		...
		int mink = ED[i].mink(); //min SFk al que ve a algun gw

		for(int j=0;j<cantInst && sinAsignar[i]>0;j++)if(solGW[i][j]==-1){ //toma una instancia sin asignar
			//cout << "i: " << i+1 << " " << j+1 << endl;
			if(cantGW == _sol[c]-1) return m+2; //si ya no puede mejorar la sol actual retorna m+2
			bool encontro = false;

			//recorre los gws factibles para ED i para cada SFk de forma random
			for(int s = mink;s<=maxk && !encontro; s++){
				//pimero mira si para ese k hay alguna ranura facible para la instancia j
				bool sirve = false;
				for(int h=j*T;h<=(j+1)*T-pow(2,s-k) && !sirve;h++) if(mapaED[i][s-k][h]) sirve = true;
				if(!sirve) continue; //si no hay ranura factible pasa al siguiente k

				vector<int> gws = ED[i].Gk(s-k);
				random_shuffle(gws.begin(), gws.end(), mirand);
				for(int ri=0; ri<gws.size() && !encontro; ri++) if(!gwabiertos[gws[ri]]){ //si el GW esta cerrado
					int r = gws[ri];
					for(int h=j*T;h<=(j+1)*T-pow(2,s-k) && !encontro;h++) if(mapaED[i][s-k][h] && mapa[r][s-k][h]==-1){
						//paso el gw a abierto
						gwabiertos[r] = true;
						cantGW++;
						...
						for(int ipp=ip;ipp<n;ipp++) if(sinAsignar[eds[ipp].second]>0 && ED[eds[ipp].second].gw(r)!=-1)
							asignarTodo(c,eds[ipp].second,r,solGW, solk, solh,mapa,mapaED,sinAsignar[eds[ipp].second],ranuras);
						encontro = true;
					}
				}
			}
...
void Heu::asignarTodo(int c,int i, int r,vector<vector<int> >& solGW, vector<vector<int> >& solk, vector<vector<int> >& solh,
	vector<vector<vector<int> > >& mapa, vector<vector<vector<bool> > >& mapaED, int& sinAsignar,vector<int>& ranuras){
        int k = _inst->k();
        int H = _inst->H();
	EndDiv ed = _inst->ED(c)[i];
	...
	for(int j=0;j<cantInst && sinAsignar>0;j++)if(solGW[i][j]==-1){ //toma una instancia sin asignar
		...
		for(int s = mink;s<=maxk && !encontro; s++){
			...
			for(int hp=0;hp<=H && !encontro;hp++) if(ranuras[hp]>=j*T && ranuras[hp]<=(j+1)*T-pow(2,s-k) &&
				...
				vector<int> ek = _inst->GWk(r,s-k,c); //EDs que ve el GW r a SFk
...
void Heu::mostrarSol(int c){
	...
	cout << "Gateways: " << endl;
	for(int j=0;j<_gwabiertos.size();j++) if(_gwabiertos[j]) cout << "\t" << j+1 << " (" << _inst->coordxGW(c,j) << "," << _inst->coordyGW(c,j) << ")" << endl;
